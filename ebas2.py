...
def to_dataframe(ungridded: pya.ungriddeddata.UngriddedData) -> pd.DataFrame:
    ...
    df["matrix"] = [
        ";".join(list(ungridded.metadata[x]["var_info"].values())[0]["matrix"])
        for x in ungridded._data[:, ungridded._METADATAKEYINDEX]
    ]
    df["component"] = [
        ";".join(list(ungridded.metadata[x]["var_info"].values())[0]["component"])
        for x in ungridded._data[:, ungridded._METADATAKEYINDEX]
    ]
    df["unit"] = [
        list(ungridded.metadata[x]["var_info"].values())[0]["units"]
        for x in ungridded._data[:, ungridded._METADATAKEYINDEX]
    ]
