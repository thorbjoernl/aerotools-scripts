...
def get_component_info(file_names: list[str], aerocom_var: str | None = None):
    ...
    for x in cur.fetchall():
        if x[2] in files:
            ...
            if (
                x[0] in ebas_components_for_aerocom_variable(aerocom_var)
                or aerocom_var is None
            ):
...
if __name__ == "__main__":
    ...
    data = read_ebas_data(
        f"{FOLDER_TO_READ}/AT0002R.19870101070000.20110303000000.wet_only_sampler..precip.1y.1d.AT01L_wados_02.AT01L_IC_a.lev2.nas"
    )
